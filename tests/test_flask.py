...
def test_create_parkings_wrong_places(client) -> None:
    ...
    assert json.loads(resp.text) == {
        "error": [
            "The number of places must be greater "
            "than the number of available places"
        ]
    }
...
def test_check_in(client) -> None:
    ...
    client.post("/api/parkings", json=parking_data)

    check_in = {"client_id": 2, "parking_id": 2}
    resp = client.post("/api/client_parkings", json=check_in)
    assert resp.status_code == 201
...
def test_check_out(client) -> None:
    ...
    check_in = {"client_id": 2, "parking_id": 2}
    client.post("/api/client_parkings", json=check_in)
    check_out = {"client_id": 2, "parking_id": 2}
    resp = client.delete("/api/client_parkings", json=check_out)
    assert resp.status_code == 200
...
def test_check_out_no_credit_cart(client) -> None:
    ...
    check_out = {
        "client_id": 2,
        "parking_id": 2,
        "time_check_out": datetime.utcnow() + timedelta(hours=2),
    }
    resp = client.delete("/api/client_parkings", json=check_out)
